...
function drilldownmenu(setting){
	this.sublevelarrow={src:'assets/images/menu-arrow.png', width:'16px', top:'8px', left:'15px'} //full URL to image used to indicate there's a sub level
	this.breadcrumbarrow='assets/images/right.gif' //full URL to image separating breadcrumb links (if it's defined)
	this.loadingimage='loader.gif' //full URL to 'loading' image, if menu is loaded via Ajax
	this.homecrumbtext='' //Top level crumb text
	this.titlelength=15 //length of parent LI text to extract from to use as crumb titles
	this.backarrow='assets/images/leftarrow.png' //full URL to image added in front of back LI 
...
drilldownmenu.prototype.buildmenu=function($){
	...
	this.$uls.each(function(i){ //loop through each UL
		...
		if (i==0){ //if topmost UL
			$('<li class="backcontroltitle"></li>').prependTo($thisul).click(function(e){e.preventDefault()})
			...
			$('<li class="backcontrol"><img src="'+thisdrill.backarrow+'" style="border-width:0" /> <span>Back</span> </li>') //add back LI item
