...
jQuery(function ($) {
	...
	var menuLib = {
		...
		'menu' : {
			 ...
			 */
			'open': function (menu) {
				menuLib.animations.slideFadeinUp($('#mobile-menu', menu).show().addClass('active'));
			 ...
			 */
			'close': function (menu) {
				menuLib.animations.slideFadeDown($("#compactanchor1.active", menu).removeClass('active'), 20, function () {
			...
			$("ul > li", $m).each(function () {
				if ($("li", $(this)).size() > 0) {
					...
					subList.attr({ 'class' : 'sub', 'id': 's' + divId})
						.prepend("<li class='previous'><a data-main=\"" + divId + "\" href=\"#\"> <img src=\"assets/images/icon-back.png\"/>&nbsp; Back</a></li>"); //Add a link to previous Element
					/**
